...
```jsonc
{
  ...
  "members": { "<clientId>": { "name": "小明", "color": "#e5484d", "joinedAt": 1690000000000,
                               ...
                               "out": 0, "outBy": "" } },  // dev/acct 认出同一个人；davId 认出同一个网盘账号；out 是退出/被移出的时间戳
...
- 「已被移出」在别人眼里只出现一次：成员面板画过一次就在本机记一笔已读（`tm:outSeen`，键是 `空间|成员`，值是这次的 `out` 时间戳），之后不再占位置；同一人日后回来再被移出一次（新的 `out`）会重新提一次。云端那条成员记录与 TA 名下的日程始终不动。
- 退出时可选「同时删除我在该空间创建的日程」：给自己名下的事件写墓碑，别的成员同步后一起看不到了。本机副本一定等云端写成功之后再清，离线时退出会明确报错，不会出现「本机已经没了、云端还不知道」。
...
- **成员**：昵称、标签色、设备身份（原生侧持久化，清缓存不换人）、可选邮箱账户（同一邮箱的多台设备并成一个用户，昵称冲突时问一句用哪个）；成员面板按 创建者 → 管理员 → 成员 → 已退出 排序，显示各自名下的日程条数与加入日期，管理员可移出普通成员
